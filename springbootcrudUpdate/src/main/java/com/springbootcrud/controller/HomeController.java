...
public class HomeController {
	...
	public String deleteFriend(@RequestParam String name, Model m) {
		List<Friends> list = fsi.delete(name);
		m.addAttribute("data", list);
		return"successPg";
	}
	
	@RequestMapping("/edit")
	public String editFriend(@RequestParam int id, Model m) {
		Friends f = fsi.getDataByID(id);
		m.addAttribute("predata", f);
		return"editPg";
	}
	
	@RequestMapping("/saveEdit")
	public String saveEditedData(@ModelAttribute Friends f) {
		fsi.updateData(f);
 		return "loginPg";
	}
